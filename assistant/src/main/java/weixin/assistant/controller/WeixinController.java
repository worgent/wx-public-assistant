...
import weixin.assistant.util.TestUtil;
 ...
 */
    ...
    public String toAuthorize(HttpServletRequest request, Model model){
        String accountID = request.getParameter("account");
        log.debug("this is log4j debug accountID," + accountID);
...
//        }
        String url = "https://mp.weixin.qq.com/cgi-bin/componentloginpage?component_appid="
                ...
                + "&redirect_uri="
                + weixinAuthService.getRedirectUrl().replace("%account%", accountID);
    ...
    public String authorizeAccessToken(HttpServletRequest request,Model model) {
        ...
        String accountID = path.replace("/call_back/", "").replace("/authorizer_access_token.do", "");
        if( accountID == null || accountID.isEmpty()){
            accountID = "0";
        }
        ...
        log.debug("authorizeAccessToken,"+accountID+","+authCode+","+expiresIn+","+ret);
            ...
            if (msgType.equals(MessageUtil.REQ_MESSAGE_TYPE_TEXT)) {
                ...
                if( appID.equals(WeixinConstant.testAppID) ) {
                    ...
                    if ( message.contains("QUERY_AUTH_CODE:")) {
                        ...
                        out.print("success");
                        out.close();
                        weixinAuthService.authorizeGzh(preAuthCode, 7200);
