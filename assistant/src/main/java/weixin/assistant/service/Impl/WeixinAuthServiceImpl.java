...
//        return null;
    ...
    public int insertWeixinGzh(WeixinGzh weixinGzh) {
        ...
        if( weixinGzhs.size() == 0 ){
             weixinGzhMapper.insertSelective(weixinGzh);
            return weixinGzh.getId();
        }else{
...
//        WeixinGzhExample weixinGzhExample = new WeixinGzhExample();
//        weixinGzhExample.createCriteria().
//                andAccountIdEqualTo(accountID).andAppIdEqualTo(appID);
//        List<WeixinGzh> weixinGzhs = weixinGzhMapper.selectByExample(weixinGzhExample);
//        if( weixinGzhs.size() == 0 ){
//            return null;
//        }else{
...
//        WeixinGzh weixinGzh = getWeixinGzh(accountID, appID);
...
//    @Override
//    public WeixinGzhAuthorizer getWeixinGzhAuthorizer(int accountID, String appID) {
//        WeixinGzh weixinGzh = getWeixinGzh(accountID, appID);
...
//    }
    ...
    @Override
    public int authorizeGzh(String authCode, long expiresIn) {
        //
        ...
        //判断是否已授权过
//        if( this.hasWeixinGzhAuthorizer(accountID, weixinGzhAuthorizer.getAppId()) ){
//            log.debug("其他公众号已经授权了，不能再授权");
//            return -4;
//        }
        WeixinGzhAuthorizer weixinGzhAuthorizerNew = this.getWeixinGzhAuthorizer(weixinGzhAuthorizer.getAppId());
        if( weixinGzhAuthorizerNew != null ){
            log.error("公众号" + weixinGzhAuthorizerNew.getAppId() + "已授权过");
            return weixinGzhAuthorizerNew.getWeixinGzhId();
        }
        //存储
        //获得个人信息
...
//        weixinGzh.setAccountId(accountID);
        ...
        int gzhID = this.insertWeixinGzh(weixinGzh);;
    ...
    public String decryptMessage(HttpServletRequest request) throws IOException, AesException {
        ...
        log.debug("timestamp,nonce,msg_signature,"+timestamp+","+nonce+","+msg_signature);
    ...
    public Map<String, String> sign(int weixinGzhID, String url) {
        WeixinGzhAuthorizer weixinGzhAuthorizer = getWeixinGzhAuthorizerByGzhID(weixinGzhID);
        String jsapiTicket = weixinGzhAuthorizer.getJsapiTicket();
        return SignUtil.sign(jsapiTicket, url);
    }

    @Override
    public String getPublicAccessToken(int gzhID) {
        WeixinGzhAuthorizer weixinGzhAuthorizer = getWeixinGzhAuthorizerByGzhID(gzhID);
        if( weixinGzhAuthorizer == null ){
            return null;
        }
        return weixinGzhAuthorizer.getAuthorizerAccessToken();
    }
